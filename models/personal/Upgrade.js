...
Upgrade.pre('save', async function (next) {
    ...
    try {
        if (isNew) {
            const { belongsTo } = await Application.findOne({ accountId })
                .select('belongsTo')
                .lean() // find application
            if (belongsTo) {
                const res = await ActiveUser.findById(belongsTo)
                    .select('email belongsTo')
                    .populate({ path: 'belongsTo', select: 'belongsTo email' })
                    .lean()
