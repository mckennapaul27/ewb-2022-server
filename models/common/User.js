...
const User = new Schema({
    ...
    referredByActiveUser: {
        // will be activeuser _id of referrer
        type: mongoose.Schema.Types.ObjectId,
        ref: 'activeuser',
    },
    // ** affpartner fields **
    ...
    referredByPartner: {
        // will be partner _id of referrer
        type: mongoose.Schema.Types.ObjectId,
        ref: 'affpartner',
    },
})
    ...
    bcrypt
        ...
        .then(async (hash) => {
            ...
            const activeUser = await createActiveUser(
                user._id,
                user.referredByActiveUser,
                user.email
            )
            const partner = await createAffPartner(
                user._id,
    ...
    return userModelCopy.findOne(
        ...
        async function (err, existingUser) {
            if (err) return next(err)
                ...
                newUser.partner = await createAffPartner(
                    newUser._id,
                    referredByPartner,
                    profile.emails[0].value
                )
                newUser.activeUser = await createActiveUser(
                    newUser._id,
                    referredByActiveUser,
                    profile.emails[0].value
                )
...
// facebook login
User.statics.upsertFbUser = function (
    referredBy,
    referredByActiveUser,
    referredByPartner,
    accessToken,
...
async function createActiveUser(belongsTo, referredBy, email) {
    // creates new activeuser, sets referredBy if applicable and pushes new active user to friends array [] of the referrer
    const newActiveUser = await ActiveUser.create({
        belongsTo,
        referredBy,
        email,
    })
    if (newActiveUser.referredBy)
        await ActiveUser.findByIdAndUpdate(
            newActiveUser.referredBy,
            { $push: { friends: newActiveUser } },
            { new: true }
        )
...
async function createAffPartner(belongsTo, referredBy, email) {
    const newAffPartner = await AffPartner.create({
        belongsTo,
        referredBy,
        email,
    })
    if (newAffPartner.referredBy)
        await AffPartner.findByIdAndUpdate(
            newAffPartner.referredBy,
            { $push: { subPartners: newAffPartner } },
            { new: true }
        )
