...
AffReport.pre('validate', async function (next) {
    // https://stackoverflow.com/questions/30141492/mongoose-presave-does-not-trigger
    try {
        const report = this
        const partner = await AffPartner.findById(report.belongsToPartner)
            .select('epi referredBy')
            .populate({ path: 'referredBy', select: 'epi' })
        report.epi = partner.epi // setting epi
        report.referredByEpi = partner.referredBy
            ? partner.referredBy.epi
            : null // setting referredByEpi
        ...
        report.quarter =
            report.brand === 'Skrill' || report.brand === 'Neteller'
                ? (await getQuarterData({ month: report.month })).quarter
