    ...
    processStatsOne.then(() => {
        ...
        processStatsTwo.then(() => {
            ...
            processStatsThree.then(() =>
                console.log('Completed activeuser data ... ')
        ...
        resolve(
            (async () => {
                ...
                const reports = await Report.find({
                    ...
                    .select(
                        'account.transValue account.accountId account.commission account.earnedFee'
                    )
                ...
                await reports.reduce(async (previousReport, nextReport) => {
                    ...
                    const { commission, earnedFee, transValue, accountId } =
                        nextReport.account
...
const getReportsVolume = async ({ _id, brand, month }) => {
...
const getSubUserVolume = async ({ _id, brand, month }) =>
    Promise.resolve(
        ActiveUser.find({ referredBy: _id })
            .select('_id')
...
const createUpdateSubReport = ({ _id }, brand, month, date) => {
    return new Promise((resolve) => {
        resolve(
            (async () => {
                const subUsers = await ActiveUser.find({
                    referredBy: _id,
                    'accounts.0': { $exists: true },
                }).select('_id epi') // { referredBy: _id } === the partner we are checking
                if (subUsers.length > 0) {
                    await subUsers.reduce(
                        async (previousSubUser, nextSubUser) => {
                            // this was previously causing the process to not run synchronously - important bit is to await it
                            ...
                            const { userId, email } = (
                                await ActiveUser.findById(nextSubUser._id)
                                    .select('belongsTo')
                                    .populate({
                                        path: 'belongsTo',
                                        select: 'userId email',
