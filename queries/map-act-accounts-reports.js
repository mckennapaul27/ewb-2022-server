const { AffApplication, AffPartner } = require('../models/affiliate/index')
const {
    Account,
    Application,
    Report,
    ...
    return new Promise((resolve) => {
        resolve(
            (async () => {
                ...
                try {
                    const isPartnerEpi = await AffPartner.findOne({ epi })
                        .select('epi')
                        .lean() // 1. If partner epi throw error
                    if (isPartnerEpi)
                        throw new Error(
                            `Account ID ${accountId} belongs to partner: ${epi}`
                        )
                    const isExistingAffApp = await AffApplication.findOne({
                        accountId,
                    })
                        .select('accountId')
                        .lean() // 2. If affapplication throw error
                    if (isExistingAffApp)
                        throw new Error(
                            `AffAccount exists for this account ID: ${accountId}`
                        )

                    const existingAccount = await Account.findOne({
                        accountId,
                    }).select('accountId reports belongsTo') // check if existing account
                    const existingApplication = await Application.findOne({
                        accountId,
                    })
                        .select('accountId belongsTo email')
                        .lean() //
                    const existingReport = await Report.findOne({
                        'account.accountId': accountId,
                        month,
                    })
                        .select('account.accountId belongsToActiveUser')
                        .lean()

                    if (existingAccount) {
                        // if account already exists in db
                        if (
                            !existingAccount.belongsTo &&
                            existingApplication &&
                            existingApplication.belongsTo
                        ) {
                            // if existing account DOES NOT belong to an activeusuer and there is an existing application which HAS a belongsTo value
                            ...
                            const activeUser =
                                await ActiveUser.findByIdAndUpdate(
                                    updatedAccount.belongsTo,
                                    { $push: { accounts: updatedAccount } },
                                    { select: 'accounts belongsTo', new: true }
                                )
                        }
                        if (existingReport) {
                            // [3-a] if existing Account and Report for the month = UPDATE IT
                            ...
                            if (
                                !existingReport.belongsToActiveUser &&
                                existingApplication &&
                                existingApplication.belongsTo
                            ) {
                                // if existing report DOES NOT belongsToActiveUser and there is an existing application which has belongsTo property
                                await Report.findByIdAndUpdate(
                                    existingReport._id,
                                    {
                                        belongsToActiveUser:
                            ...
                            // [3-b] if existing Account and if NO existing report for the month = CREATE IT
                        ...
                        // if account does not exist in db
                        // [4] if no existing account but valid existingApplication - create account and report
                        if (
                            existingApplication &&
                            existingApplication.belongsTo
                        ) {
                            // if existingApplication does exist in db and it belongs to an activeuser
                            ...
                            // [5] if no Application and no account or report - create without belongsToActiveUser for this accountId
                            const newAccount = await Account.create({
                                // create new account
                                accountId,
                                brand,
