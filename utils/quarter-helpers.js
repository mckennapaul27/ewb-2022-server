                    ...
                    if (existingQuarter ) { // if existing quarter 
                        ...
                        if (level && (quarter === currentQuarter)) { // if transvalue qualfies for an upgrade && quarter is equal to the current quarter  - if it is not current quarter, it will delete all affupgrades from different quarters. only affupgrades in the current quarter are valid
                            ...
                            if (!existingLevelUpgrade) { // if existingLevelUpgrade for THIS LEVEL does not exist
                                let affApplication = await AffApplication.findOne({ accountId }).select('_id accountId').lean(); // find affApplication
                                if (!affApplication) { // Having affapplication is essential to process so if it is not found, we need to create.
                                    affApplication = await AffApplication.create({ brand, accountId, belongsTo: _id, status: 'Approved' })
                                };
                                ...
                                const a = await AffUpgrade.create({ // create newAffUpgrade which we can use for isNew to send email
                                    level,
                                    quarter,
                                    accountId,
                                    brand,
                                    belongsTo: affApplication, // need this to find partner detaiils
                                    startDate,
                                    endDate
                                });
                                ...
                                await AffApplication.findByIdAndUpdate(affApplication._id, { // update affapplication so that partner can request it
                                    availableUpgrade,
