        ...
        resolve(
            (async () => {
                try {
                    ...
                    const commission = reports.reduce(
                        (acc, item) => ((acc += item.commission), acc),
                        0
                    )
                    const cashback = reports.reduce(
                        (acc, item) => ((acc += item.cashback), acc),
                        0
                    )
                    const rafCommission = subReports.reduce(
                        (acc, item) => ((acc += item.total), acc),
                        0
                    )
                    ...
                    const requested = payments.reduce(
                        (acc, item) =>
                            item._id.status === 'Requested'
                                ? ((acc += item.total), acc)
                                : acc,
                        0
                    )
        ...
        resolve(
            (async () => {
                ...
                const affSubReports = await AffSubReport.aggregate([
                    {
                        $match: {
                            $and: [{ belongsTo: mongoose.Types.ObjectId(_id) }],
                        },
                    },
                ...
                const commission = affReports.reduce(
                    (acc, item) => (
                        (acc[item._id.currency] += item.commission), acc
                    ),
                    { USD: 0, EUR: 0 }
                )
                const cashback = affReports.reduce(
                    (acc, item) => (
                        (acc[item._id.currency] += item.cashback), acc
                    ),
                    { USD: 0, EUR: 0 }
                )
                const subCommission = affSubReports.reduce(
                    (acc, item) => (
                        (acc[item._id.currency] += item.total), acc
                    ),
                    { USD: 0, EUR: 0 }
                )
                const paid = affPayments.reduce(
                    (acc, item) =>
                        item._id.status === 'Paid'
                            ? ((acc[item._id.currency] += item.total), acc)
                            : acc,
                ...
                const requested = affPayments.reduce(
                    (acc, item) =>
                        item._id.status === 'Requested'
                            ? ((acc[item._id.currency] += item.total), acc)
                            : acc,
                ...
                let balance = {
                    USD:
                        cashback['USD'] +
                        subCommission['USD'] -
                        (paid['USD'] + requested['USD']),
                    EUR:
                        cashback['EUR'] +
                        subCommission['EUR'] -
                        (paid['EUR'] + requested['EUR']),
                }
                ...
                await ['USD', 'EUR'].reduce(async (acc, currency) => {
                    ...
                    const partner = await AffPartner.findByIdAndUpdate(
                        _id,
                        {
                            ...
                            'stats.subCommission.$[el].amount':
                                subCommission[currency], // Currently the affpartners we have in local db does not include subCommission in stats array - for this reason it may fail. When we load data from old site, we need to make sure every partner has stats.subCommission in their balance array
                        },
                        {
                            new: true,
