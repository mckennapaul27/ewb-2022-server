...
function updateBalances (req, res) { // After next() is called on createPayment() it comes next to updateBalances()
    return Promise.all([
        ...
        Report.aggregate([
            { $match: { $and: [ { belongsToActiveUser: mongoose.Types.ObjectId(req.params._id) }, { currency: req.body.currency } ] } }, // need to choose currency to update that balance
