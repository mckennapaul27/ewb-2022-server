...
router.get(
    ...
    async (req, res) => {
        const token = getToken(req.headers)
        if (token) {
            const vipRequest = await AffApplication.findByIdAndUpdate(
                req.params._id,
                {
...
router.get(
    ...
    async (req, res) => {
        ...
        const token = getToken(req.headers)
        if (token) {
            const existsOne = await Application.countDocuments({ accountId })
                .select('accountId')
                .lean()
            const existsTwo = await AffApplication.countDocuments({ accountId })
                .select('accountId')
                .lean()
...
async function getApplications(req, res) {
    ...
    if (token) {
        ...
        try {
            const applications = await AffApplication.find(query)
...
router.post(
    ...
    async (req, res) => {
        ...
        if (token) {
            ...
            await AffApplication.findOneAndUpdate(
                { accountId },
                {
                    upgradeStatus: `Requested ${dayjs().format('DD/MM/YYYY')}`,
                    'availableUpgrade.valid': false,
