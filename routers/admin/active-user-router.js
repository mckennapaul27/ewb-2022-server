...
router.post(
    ...
    async (req, res) => {
        ...
        if (token) {
            try {
                const partner = await ActiveUser.findById(req.body._id)
...
router.post(
    ...
    async (req, res) => {
        ...
        if (token) {
            try {
                const partner = await ActiveUser.findByIdAndUpdate(
                    req.params._id,
                    update,
                    { new: true }
                )
    ...
    async (req, res) => {
        ...
        if (token) {
            ...
            try {
                ...
                const allTotals = await Report.aggregate([
    ...
    async (req, res) => {
        ...
        if (token) {
            try {
                ...
                const aa = await Application.findByIdAndUpdate(
                    req.params._id,
                    update
                ...
                const {
                    ...
                    currency,
                    availableUpgrade,
                } = aa // deconstruct updated application
                const activeUser = await ActiveUser.findById(belongsTo)
                    .select('belongsTo email')
                    .populate({ path: 'belongsTo', select: '_id locale' })
                ...
                if ((action === 'YY' || action === 'YN') && belongsTo)
                    await createAccountReport({ accountId, brand, belongsTo }) // create affaccount and affreport if not already created (Only if YY or YN)
        ...
        if (token) {
            ...
            try {
                if (isPopulatedValue(query)) {
                    payments = (
                        await Payment.find(searchQuery)
                            .collation({ locale: 'en', strength: 1 })
                            .sort(sort)
                            .skip(skippage)
                            .limit(pageSize)
                    ...
                    ).filter((a) => a.belongsTo.belongsTo)
                } else {
                    payments = await Payment.find(searchQuery)
                        ...
                        .populate({
                            path: 'belongsTo',
                            select: 'belongsTo',
                            populate: { path: 'belongsTo', select: 'userId' },
                        })
                }
        ...
        if (token) {
            ...
            try {
                ...
                let activeUser = await ActiveUser.findById(belongsTo)
                    .select('belongsTo email')
                    .populate({ path: 'belongsTo', select: 'locale' }) // get the _id of the user that activeuser belongsTo
                createUserNotification(
                    paymentResult({
                        symbol: currency === 'USD' ? '$' : '€',
                        amount,
